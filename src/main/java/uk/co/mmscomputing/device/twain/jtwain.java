...
//  static         native byte[] ncopy(byte[] twImageMemXfer,int size);         // copy native memory block into java byte array
  ...
  static public boolean setScanner(TwainScanner s)throws TwainIOException{ // use of weak reference to TwainScanner object
...
//    installed = JarLib.load(jtwain.class,"jtwain");
    TwainNativeLoadStrategySingleton instance = TwainNativeLoadStrategySingleton.getInstance();
    TwainINativeLoadStrategy strategy = instance.getNativeLoadStrategy();
    installed = strategy.load(jtwain.class, "jtwain");
